...
const CryptoDetails = () => {
    ...
    const i = location.pathname.lastIndexOf('/') + 1
    let coinId = location.pathname.slice(i)
        ...
        <Col className="coin-detail-container">
            ...
            <Col className="stats-container">
                <Col className="coin-value-statistics">
                    <Col className="coin-value-statistics-heading">
                        <Typography.Title level={3} className="coin-details-heading">{cryptoDetails.name} Value
                            Statistics</Typography.Title>
                    ...
                    {stats.map(({icon, title, value}, i: number) => (
                ...
                <Col className="other-stats-info">
                    ...
                    {genericStats.map(({icon, title, value}, i: number) => (
                        <Col key={i} className="coin-stats">
                            <Col className="coin-stats-name">
                                <Typography.Text>{icon}</Typography.Text>
                                <Typography.Text>{title}</Typography.Text>
            ...
            <Col className="coin-desc-link">
                <Row className="coin-desc">
                    <Typography.Title level={3} className="coin-details-heading">What
                        is {cryptoDetails.name}?</Typography.Title>
                    {HTMLReactParser(cryptoDetails.description)}
                </Row>
                <Col className="coin-links">
                    <Typography.Title level={3}
                                      className="coin-details-heading">{cryptoDetails.name} Links</Typography.Title>
